...
// Command keyboard is a desktop front end for a pico-hid board: a full-size
// on-screen keyboard, and a trackpad beside it, driving the target machine
// through the board's HTTP API.
//
//	./keyboard.sh
//
// The board address is set under Settings… in the menu bar and remembered
// between runs. It can also come from -url or PICO_HID_SERVER_URL (or .env).
package main
...
// Sending
//
// The board serves one connection at a time and isn't listening while it holds
// a key down, so overlapping requests get refused — which shows up as dropped
// keystrokes. Everything therefore funnels through a single worker that sends
// one command at a time with a gap in between, exactly like the queue in
// public/index.html. The two clients speak the same protocol, including the
// seq de-duplication token.
// ---------------------------------------------------------------------------

const (
	sendGap     = 70 * time.Millisecond // must exceed the board's per-key hold time
...
const (
	appName     = "Pico HID Keyboard"
	appID       = "dev.picohid.keyboard"
	prefBoardID = "board-id"
	prefSpeed   = "pointer-speed"
	prefTarget  = "target-os"
)

// The board answers on the name code.py advertises over mDNS: "ph-" followed by
// four hex digits of its CPU id. Only that id varies, so it is all the settings
// ask for and all that gets stored. The board prints its id on boot and writes
// it to hostname.txt on the CIRCUITPY drive.
const (
	boardHostPrefix = "http://ph-"
	boardHostSuffix = ".local"
	boardIDHint     = "xxxx"
	// An id is always this long: code.py builds it from two bytes of the CPU id
	// as lowercase hex.
	boardIDLength = 4
)

// boardURL is the address of the board with this id, or "" if there isn't one
// yet.
func boardURL(id string) string {
	if id = normalizeBoardID(id); id == "" {
		return ""
	}
	return boardHostPrefix + id + boardHostSuffix
}

// normalizeBoardID reduces what was typed to an id the board could actually
// have: hex digits, lower case, and never more than four of them. It tolerates a
// whole address being pasted in, since that is the obvious thing to try.
func normalizeBoardID(s string) string {
	var id strings.Builder
	for _, r := range strings.ToLower(cleanBoardID(s)) {
		if id.Len() == boardIDLength {
			break
		}
		if (r >= '0' && r <= '9') || (r >= 'a' && r <= 'f') {
			id.WriteRune(r)
...
// limitToBoardID keeps an entry's text down to something a board id could be.
// Fyne's Entry has no length limit of its own, so over-long or non-hex input is
// put back trimmed — which re-enters this handler once and then settles.
func limitToBoardID(entry *widget.Entry, apply func(id string)) func(string) {
	return func(s string) {
		if id := normalizeBoardID(s); id != s {
			entry.SetText(id)
			return
		}
		apply(s)
...
// boardIDIn is the id an address names, or "" if the address isn't of the
// board's own form. A -url pointing at an IP has no id to show, and hex digits
// picked out of one would only mislead.
func boardIDIn(address string) string {
	host := strings.TrimSpace(address)
	host = strings.TrimPrefix(host, "https://")
	host = strings.TrimPrefix(host, "http://")
	host = strings.TrimSuffix(host, "/")
	host = strings.TrimSuffix(host, boardHostSuffix)
	id, ok := strings.CutPrefix(host, "ph-")
	if !ok {
		return ""
	}
...
// cleanBoardID reduces whatever was typed or pasted to the id itself, so a whole
// address pasted into the field still lands on the right board.
func cleanBoardID(s string) string {
	s = strings.TrimSpace(s)
	s = strings.TrimPrefix(s, "https://")
	s = strings.TrimPrefix(s, "http://")
	s = strings.TrimSuffix(s, "/")
	s = strings.TrimSuffix(s, boardHostSuffix)
	return strings.TrimPrefix(s, "ph-")
}

// Which machine the board is plugged into. It changes no key that gets sent —
// the board always sends physical keys — only how the modifiers are labelled
// and which side of the space bar each one sits on.
const (
...
// addrCache remembers what the board's name resolved to. Resolving an mDNS name
// can take seconds — five, on a network whose router answers ".local" queries
// slowly — and the cost lands on every single command, because the board closes
// each connection so there is nothing to keep alive. Resolve once, reuse, and
// look it up again only if connecting to the remembered address fails.
...
func (c *addrCache) dial(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if known != "" {
		...
		// The board may have been given a different address; forget and re-look.
		c.mu.Lock()
...
func newSender(boardURL string) *sender {
	addrs := &addrCache{
		dialer: net.Dialer{Timeout: sendTimeout},
		known:  map[string]string{},
	}
	return &sender{
		// The board answers with "Connection: close", so pooling buys nothing
		// and a half-dead idle connection would only cost a lost command. The
		// address it dials is cached instead, which is where the time goes.
...
// resolve looks the board up now, so the wait doesn't land on the first
...
func (s *sender) setURL(u string) {
	...
	s.mu.Unlock()
	if changed {
		s.addrs.forget() // a different board, so a different address
	}
}
...
// retry can re-send a command the board already ran (the response was lost,
// not the request). The token is stamped once and reused on retry, and the
// board skips a token it just handled, so a keystroke can't come out doubled.
...
func (s *sender) post(body string) error {
	...
	s.mu.Unlock()
	if target == "" {
		return errors.New("no board address set")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("board returned HTTP %d", resp.StatusCode)
	}
	return nil
}

// normalizeURL turns what someone is likely to have to hand — "ph-3f9a.local",
// "192.168.1.20", "http://board/" — into an address the board answers on.
// Commands are POSTed to "/", the same endpoint the web UI uses.
...
// never reaches these handlers either, so a held key can't flood the board.
// ---------------------------------------------------------------------------
...
func (r *keyboardPanelRenderer) Layout(size fyne.Size) {
	p := r.panel
	unit := size.Width / p.cols
	if byHeight := size.Height / p.rows; byHeight < unit {
		unit = byHeight
	}
	offX := (size.Width - p.cols*unit) / 2
	offY := (size.Height - p.rows*unit) / 2
	if offY < 0 {
		offY = 0
	}
	for _, pl := range p.placed {
...
func (r *keyboardPanelRenderer) MinSize() fyne.Size {
	return fyne.NewSize(r.panel.cols*minKeyUnit, r.panel.rows*minKeyUnit)
}
...
func main() {
	urlFlag := flag.String("url", "", "board address, e.g. http://ph-3f9a.local")
	flag.Parse()

	// The repo's other client reads client/.env, so read it here too and one
	// file configures both. A missing .env is not a problem: the address can
	// come from the flag, the environment, or the settings dialog.
	_ = godotenv.Load()
	...
	// A flag or the environment can name any host — an IP, say — while the
	// settings field only spells out the id, which covers every board this
	// firmware builds a name for.
	address := firstNonEmpty(*urlFlag, os.Getenv("PICO_HID_SERVER_URL"),
		boardURL(prefs.String(prefBoardID)))
	configured := address != ""
	snd := newSender(normalizeURL(address))

	// Which machine the board is plugged into is anyone's guess, so start from
	// the one running this window and let the setting correct it.
	ui := &keyboardUI{
		snd:     snd,
		mods:    map[string]int{},
	...
	settings := func() {
		// Only the id is editable: the rest of the address is the same for every
		// board, so spelling it out around the field says what shape it takes.
		entry := widget.NewEntry()
		entry.SetPlaceHolder(boardIDHint)
		// An Entry scrolls its text, and the scroller draws a shadow at the edge
		// once it thinks the content overruns the box — which a box this narrow
		// always does. Four hex digits can't overflow it, so the scrolling is no
		// loss and the shadow goes with it.
		entry.Wrapping = fyne.TextWrapOff
		entry.Scroll = fyne.ScrollNone
		entry.SetText(boardIDIn(address))
		entry.OnChanged = limitToBoardID(entry, func(id string) {
			address = boardURL(id)
			snd.setURL(normalizeURL(address))
			prefs.SetString(prefBoardID, id)
		})
		...
		d := dialog.NewCustom("Settings", "Close", widget.NewForm(
			widget.NewFormItem("Board", container.NewHBox(
				widget.NewLabel(boardHostPrefix),
				container.NewGridWrap(fyne.NewSize(boardIDWidth, entry.MinSize().Height), entry),
				widget.NewLabel(boardHostSuffix),
			)),
		...
		d.Show()
		// Fyne doesn't focus a dialog's first field, and without focus the board
		// id would have nowhere to land.
